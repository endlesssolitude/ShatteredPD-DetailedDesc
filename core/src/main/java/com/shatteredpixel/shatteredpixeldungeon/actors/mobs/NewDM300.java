...
public class NewDM300 extends Mob {
	...
	public int attackProc(Char enemy, int damage){
		if(Dungeon.isChallenged(Challenges.ELITE_BOSSES)){
			...
			if(Random.Int(4)==0){
				Buff.prolong(enemy, Vulnerable.class, 8f);
				Buff.prolong(enemy, Cripple.class, 8f);
				Buff.prolong(enemy, Blindness.class, 8f);
			}
		}
	...
	public void ventGas( Char target ){
		...
		if(Dungeon.isChallenged(Challenges.ELITE_BOSSES)){
			GameScene.add(Blob.seed(trajectory.collisionPos, 100, CorrosiveGas.class));
		}
		...
		if (gasVented < 250){
			...
			for (int i : PathFinder.NEIGHBOURS8){
				...
				if(Dungeon.isChallenged(Challenges.ELITE_BOSSES)){
					GameScene.add(Blob.seed(pos+i, toVentAround, CorrosiveGas.class));
				}
			}
