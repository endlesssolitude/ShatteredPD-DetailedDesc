	...
	private static class CatalogTab extends Component{
		...
		private void updateList() {
			...
			if (currentItemIdx == ARMORS_IDX) {
				itemClasses = new ArrayList<>(Catalog.ARMORS.items());
				...
				itemClasses = new ArrayList<>(Catalog.ALCHEMY.items());
				for (Class<? extends Item> cls : itemClasses) known.put(cls, true);
			} else if (currentItemIdx == MOB_IDX){
				itemClasses = new ArrayList<>(Catalog.MOBS.items());
				for (Class<? extends Item> cls : itemClasses) known.put(cls, true);
			} else if (currentItemIdx == PLANTS_IDX) {
				itemClasses = new ArrayList<>(Catalog.PLANTS.items());
				for (Class<? extends Item> cls : itemClasses) known.put(cls, true);
			} else if (currentItemIdx == UNCLASSIFIED_IDX) {
				itemClasses = new ArrayList<>(Catalog.UNCLASSIFIED.items());
				for (Class<? extends Item> cls : itemClasses) known.put(cls, true);
			} else if (currentItemIdx == WANDS_IDX) {
				itemClasses = new ArrayList<>(Catalog.WANDS.items());
				for (Class<? extends Item> cls : itemClasses) known.put(cls, true);
			} else if (currentItemIdx == DOCUMENTS_IDX) {
				itemClasses = new ArrayList<>(Catalog.DOCUMENTS.items());
				for (Class<? extends Item> cls : itemClasses) known.put(cls, true);
			} else {
