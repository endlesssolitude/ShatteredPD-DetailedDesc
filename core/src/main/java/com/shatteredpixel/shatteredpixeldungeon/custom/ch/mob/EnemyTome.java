...
public class EnemyTome extends ChallengeItem {
    {
        unique = true;
    ...
    public static class SettingsWindow extends Window {
        ...
        public SettingsWindow(){
            ...
            RenderedTextBlock rtb = PixelScene.renderTextBlock(M.L(EnemyTome.class, "title"), TTL_HEIGHT - GAP);
            ...
            cbs = new ArrayList<>();
            for(int i = 0; i<5; ++i){
                CheckBox cb = new CheckBox(M.L(EnemyTome.class, "chapter_"+String.valueOf(i+1)));
                cb.setRect(GAP, pos, WIDTH - GAP * 2, BOX_HEIGHT);
                add(cb);
                cbs.add(cb);
                pos += BOX_HEIGHT + GAP;
                cb.checked((Statistics.elite_enemies & (1<<i)) >0);
            }
        ...
        public void onBackPressed() {
            Statistics.elite_enemies = 0;
            for(int i=0, len = cbs.size(); i<len; ++i){
                if(cbs.get(i).checked()) {
                    Statistics.elite_enemies += 1<<i;
                }
            }
            super.onBackPressed();
        }
